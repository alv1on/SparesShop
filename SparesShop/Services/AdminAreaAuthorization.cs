...
namespace SparesShop.Services
{
    public class AdminAreaAuthorization : IControllerModelConvention    
    {
        ...
        public void Apply(ControllerModel controller) // метод apply который создает фильтры для ролей
        {
            if (controller.Attributes.Any(a => a is AreaAttribute && (a as AreaAttribute).RouteValue.Equals(_area, StringComparison.OrdinalIgnoreCase))
                || controller.RouteValues.Any(r => r.Key.Equals("area", StringComparison.OrdinalIgnoreCase) && r.Value.Equals(_area, StringComparison.OrdinalIgnoreCase)))
            {
